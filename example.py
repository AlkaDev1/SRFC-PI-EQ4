...
#esta vista es el ejemplo de gestion, posterior a esto se pasará al archivo original
...
def app() -> None:
    ...
    #boton inferior para regresar a la pantalla principal
    def regresar_a_main() -> None:
        root.destroy()
        try:
            from src.views.main import abrir_principal
        except ModuleNotFoundError:
            from main import abrir_principal
        abrir_principal()

    ttk.Button(contenido, text="< Regresar", style="Ghost.TButton", command=regresar_a_main).pack(anchor="w", pady=(24, 0))
